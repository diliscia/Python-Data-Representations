...
def reconcile_countries_by_name(plot_countries, gdp_countries):
    ...
    for code in plot_countries:
        if not plot_countries.get(code) in gdp_countries:
            ...
            country_data[code] = plot_countries.get(code)
            
    tupled_values = () + (country_data, country_code,)
        ...
        for data in reader:
            for country in plot_countries:
                if plot_countries.get(country) in data.values():
                    try:
                        country_code[country] = math.log10(float(data.get(year)))
                    except ValueError:
                        no_value_countries.add(country)
                    
    for country in plot_countries:
        if not country in country_code and not country in no_value_countries:
            no_code.add(country)
        
    tupled_values = () + (country_code, no_code, no_value_countries, )
    return tupled_values
...
def render_world_map(gdpinfo, plot_countries, year, map_file):
    ...
    worldmap_chart.add('GDP For ' + year, gdp_data[0])
    worldmap_chart.add('Missing from World Bank Data', gdp_data[1])
    worldmap_chart.add('No GDP Data', gdp_data[2])
